...
export function ProductCard(props: ProductCardProps) {
  console.log(props)
  const variant = props.variants.nodes.find(Boolean)?.price
  const href = `/products/${props.handle}`
  const linkAria = `Visit product: ${props.title}`
  const featuredImageAltTag = props.images.nodes.find((singleImage) => singleImage.url === props.featuredImage?.url)?.altText || ""
